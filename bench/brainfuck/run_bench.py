...
def main():
    ...
    if should_run('Dart'):
        if shutil.which("dart"):
            dart_ok = compile_and_show_output("Dart", ["dart", "compile", "exe", "dart/main.dart", "-o", "dart/bf"])
        else:
    ...
    if should_run('Haskell'):
        if shutil.which("ghc"):
            haskell_ok = compile_and_show_output("Haskell", ["ghc", "-O2", "haskell/Main.hs", "-o", "haskell/bf"])
        else:
    ...
    if should_run('OCaml'):
        if shutil.which("ocamlopt"):
            ocaml_ok = compile_and_show_output("OCaml", ["ocamlopt", "-O3", "ocaml/main.ml", "-o", "ocaml/bf"])
        else:
    ...
    targets = [
        {"name": "Haskell", "cmd": ["./haskell/bf"], "available": haskell_ok},
        {"name": "OCaml", "cmd": ["./ocaml/bf"], "available": ocaml_ok},
        ...
        {"name": "Dart", "cmd": ["./dart/bf"], "available": dart_ok},
