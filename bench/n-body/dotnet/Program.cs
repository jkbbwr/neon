...
OffsetMomentum();
string before = Energy().ToString("F9", CultureInfo.InvariantCulture);
Console.WriteLine(before);
for (int i = 0; i < N; i++) Advance(0.01);
string after = Energy().ToString("F9", CultureInfo.InvariantCulture);
Console.WriteLine(after);
Console.WriteLine($"Result: {before} {after}");

class Body(double x, double y, double z, double vx, double vy, double vz, double mass)
{
    public double x = x, y = y, z = z, vx = vx, vy = vy, vz = vz, mass = mass;
}
