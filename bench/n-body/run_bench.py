...
def run_and_time(cmd):
    ...
    if "Result: -0.169075164 -0.169031665" not in res.stdout:
...
def main():
    ...
    if should_run('C'):
        if shutil.which("gcc"):
            c_ok = compile_and_show_output("C", ["gcc", "-O3", "c/main.c", "-o", "c/nb", "-lm"])
        else:
    ...
    if should_run('C++'):
        if shutil.which("g++"):
            cpp_ok = compile_and_show_output("C++", ["g++", "-O3", "cpp/main.cpp", "-o", "cpp/nb"])
        else:
    ...
    if should_run('Zig'):
        if shutil.which("zig"):
            zig_ok = compile_and_show_output("Zig", ["zig", "build-exe", "-O", "ReleaseFast", "zig/main.zig", "-femit-bin=zig/nb"])
        else:
    ...
    if should_run('C# (.NET)'):
        if shutil.which("dotnet"):
            dotnet_ok = compile_and_show_output("C# (.NET)", ["dotnet", "publish", "dotnet/nbody.csproj", "-c", "Release", "-o", "dotnet/build"])
    ...
    if should_run('Java (Native)'):
        if java_ok and shutil.which("native-image"):
            java_native_ok = compile_and_show_output("Java (Native)", ["native-image", "-cp", "java", "-O3", "Main", "-o", "java/nb_native"])
    ...
    if should_run('Clojure (Native)'):
        if clojure_ok and shutil.which("native-image") and shutil.which("java"):
            try:
                ...
                if aot_ok:
                    clojure_native_ok = compile_and_show_output("Clojure (Native)", ["native-image", "-cp", f"{clojure_cp}:clojure/classes", "-O3", "--no-fallback", "--initialize-at-build-time", "main", "-o", "clojure/nb_native"])
    ...
    targets = [
        ...
        {"name": "Neon", "cmd": ["./neon/_neon/nbody"], "available": neon_ok},
        ...
        {"name": "Clojure (Native)", "cmd": ["./clojure/nb_native"], "available": clojure_native_ok},
    ...
    table = Table(title="N-Body Benchmarks", show_header=True, header_style="bold magenta")
