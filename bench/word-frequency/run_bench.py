...
def run_and_time(cmd):
    ...
    if "Result: 10000 10000000 1139" not in res.stdout:
...
def main():
    ...
    if should_run('C'):
        if shutil.which("gcc"):
            c_ok = compile_and_show_output("C", ["gcc", "-O3", "c/main.c", "-o", "c/wf"])
        else:
    ...
    if should_run('C++'):
        if shutil.which("g++"):
            cpp_ok = compile_and_show_output("C++", ["g++", "-O3", "cpp/main.cpp", "-o", "cpp/wf"])
        else:
    ...
    if should_run('Zig'):
        if shutil.which("zig"):
            zig_ok = compile_and_show_output("Zig", ["zig", "build-exe", "-O", "ReleaseFast", "zig/main.zig", "-femit-bin=zig/wf"])
        else:
    ...
    if should_run('Go'):
        if shutil.which("go"):
            go_ok = compile_and_show_output("Go", ["go", "build", "-o", "go/wf", "go/main.go"])

    # Compile Dart
    dart_ok = False
    if should_run('Dart'):
        if shutil.which("dart"):
            dart_ok = compile_and_show_output("Dart", ["dart", "compile", "exe", "dart/main.dart", "-o", "dart/wf"])
        else:
    ...
    if should_run('C# (.NET)'):
        if shutil.which("dotnet"):
            dotnet_ok = compile_and_show_output("C# (.NET)", ["dotnet", "publish", "dotnet/wordfreq.csproj", "-c", "Release", "-o", "dotnet/build"])
    ...
    if should_run('Haskell'):
        if shutil.which("ghc"):
            haskell_ok = compile_and_show_output("Haskell", ["ghc", "-O2", "haskell/Main.hs", "-o", "haskell/wf"])
        else:
    ...
    if should_run('OCaml'):
        if shutil.which("ocamlopt"):
            ocaml_ok = compile_and_show_output("OCaml", ["ocamlopt", "-O3", "ocaml/main.ml", "-o", "ocaml/wf"])
        else:
    ...
    if should_run('Java (Native)'):
        if java_ok and shutil.which("native-image"):
            java_native_ok = compile_and_show_output("Java (Native)", ["native-image", "-cp", "java", "-O3", "Main", "-o", "java/wf_native"])
    ...
    if should_run('Clojure (Native)'):
        if clojure_ok and shutil.which("native-image") and shutil.which("java"):
            try:
                ...
                if aot_ok:
                    clojure_native_ok = compile_and_show_output("Clojure (Native)", ["native-image", "-cp", f"{clojure_cp}:clojure/classes", "-O3", "--no-fallback", "--initialize-at-build-time", "main", "-o", "clojure/wf_native"])
            except Exception as e:
                console.print(f"[red]Failed to compile Clojure (Native): {e}[/red]")

    targets = [
        {"name": "Haskell", "cmd": ["./haskell/wf"], "available": haskell_ok},
        {"name": "OCaml", "cmd": ["./ocaml/wf"], "available": ocaml_ok},
        {"name": "C", "cmd": ["./c/wf"], "available": c_ok},
        {"name": "C++", "cmd": ["./cpp/wf"], "available": cpp_ok},
        {"name": "Zig", "cmd": ["./zig/wf"], "available": zig_ok},
        {"name": "Rust", "cmd": ["./rust/target/release/wordfreq"], "available": rust_ok},
        {"name": "Go", "cmd": ["./go/wf"], "available": go_ok},
        {"name": "Dart", "cmd": ["./dart/wf"], "available": dart_ok},
        {"name": "C# (.NET)", "cmd": ["./dotnet/build/wordfreq"], "available": dotnet_ok},
        {"name": "Java (Native)", "cmd": ["./java/wf_native"], "available": java_native_ok},
        ...
        {"name": "Neon", "cmd": ["./neon/_neon/wordfreq"], "available": neon_ok},
        ...
        {"name": "Clojure (Native)", "cmd": ["./clojure/wf_native"], "available": clojure_native_ok},
    ...
    table = Table(title="Word-Frequency Benchmarks", show_header=True, header_style="bold magenta")
